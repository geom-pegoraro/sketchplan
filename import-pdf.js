...
function initImportScreen() {
  ...
  // Annulla ultimo punto inserito (torna indietro di un passo)
  const btnUndo = document.getElementById('btn-calib-undo');
  if (btnUndo) {
    btnUndo.addEventListener('click', () => {
      if (importState.importPhase === 'calib-input' || importState.importPhase === 'pick2') {
        // Annulla punto 2 → torna a pick2
        importState.calibPt2 = null;
        document.getElementById('import-calib-panel').classList.remove('visible');
        setImportPhase('pick2');
        renderImportCanvas();
        showImportToast('↩ Punto 2 annullato — clicca il SECONDO punto.', 'info', 4000);
      } else if (importState.importPhase === 'pick2') {
        // Annulla punto 1 → torna a pick1
        importState.calibPt1 = null;
        importState.calibPt2 = null;
        setImportPhase('pick1');
        renderImportCanvas();
        showImportToast('↩ Punto 1 annullato — clicca il PRIMO punto.', 'info', 4000);
      }
    });
  }
...
// Soglia massima di movimento (px) e durata (ms) per considerare un gesto un TAP
...
function onImportTouchStart(e) {
  ...
  if (e.touches.length === 1) {
    ...
    // Due dita → sicuramente non un tap, cancella ogni intenzione di tap
    _didPan = true;
...
function onImportTouchMove(e) {
  ...
  if (e.touches.length === 1 && importState._panStart) {
    ...
    // Considera pan solo se mosso oltre la soglia TAP
    if (!_didPan && (Math.abs(dx) > TAP_MAX_MOVE || Math.abs(dy) > TAP_MAX_MOVE)) {
      _didPan = true;
    }
    if (_didPan) {
      importState.view.offX = pos.x - importState._panStart.x;
...
function onImportTouchEnd(e) {
  ...
  // È un tap solo se: dito singolo, non si è mosso oltre soglia, durata breve
  const wasTap = !_didPan && elapsed < TAP_MAX_MS && importState._lastTouches.length === 1;
  ...
  // Se restano ancora dita → aggiorna
