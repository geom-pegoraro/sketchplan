...
function initImportScreen() {
  ...
  // Annulla ultimo punto inserito
  const btnUndoPt = document.getElementById('btn-calib-undo');
  if (btnUndoPt) {
    btnUndoPt.addEventListener('click', () => {
      if (importState.importPhase === 'calib-input') {
        importState.calibPt2 = null;
        document.getElementById('import-calib-panel').classList.remove('visible');
        setImportPhase('pick2');
        renderImportCanvas();
        showImportToast('↩ Punto 2 annullato — clicca il SECONDO punto.', 'info', 3500);
      } else if (importState.importPhase === 'pick2') {
        importState.calibPt1 = null;
        importState.calibPt2 = null;
        setImportPhase('pick1');
        renderImportCanvas();
        showImportToast('↩ Punto 1 annullato — clicca il PRIMO punto.', 'info', 3500);
      }
    });
  }
...
function activateOverlay() {
  ...
  // Attiva Seleziona e aggiorna UI toolbar
  if (typeof window.setActiveTool === 'function') {
    window.setActiveTool('select');
  } else {
    state.activeTool = 'select';
    document.querySelectorAll('[data-tool]').forEach(b => b.classList.remove('active'));
    const sb = document.querySelector('[data-tool="select"]');
...
// Costanti per il rilevamento tap vs pan
const _TAP_MAX_MOVE = 10; // px
const _TAP_MAX_MS   = 280; // ms
...
function onImportTouchStart(e) {
  ...
  if (e.touches.length === 1) {
    ...
    // Due dita: azzera panStart per evitare salti al ritorno a 1 dito
    _didPan = true;
    importState._panStart  = null;
    importState._pinchDist = pinchDist(e.touches[0], e.touches[1]);
    // Salva midpoint per pan simultaneo al pinch
    importState._pinchMid = {
      x: (getTouchCanvasPos(e.touches[0]).x + getTouchCanvasPos(e.touches[1]).x) / 2,
      y: (getTouchCanvasPos(e.touches[0]).y + getTouchCanvasPos(e.touches[1]).y) / 2,
    };
  }
}
...
function onImportTouchMove(e) {
  ...
  if (e.touches.length === 1) {
    ...
    // Se _panStart è null (venivamo da pinch) ricalcolalo ora senza salti
    if (!importState._panStart) {
      importState._panStart = {
        x: pos.x - importState.view.offX,
        y: pos.y - importState.view.offY,
      };
    ...
    const dx = pos.x - (_mouseDownPos ? _mouseDownPos.x : pos.x);
    const dy = pos.y - (_mouseDownPos ? _mouseDownPos.y : pos.y);
    if (!_didPan && (Math.abs(dx) > _TAP_MAX_MOVE || Math.abs(dy) > _TAP_MAX_MOVE)) {
      _didPan = true;
    }
    if (_didPan && importState._panStart) {
      importState.view.offX = pos.x - importState._panStart.x;
      importState.view.offY = pos.y - importState._panStart.y;
    }

  } else if (e.touches.length === 2) {
    const t0 = getTouchCanvasPos(e.touches[0]);
    const t1 = getTouchCanvasPos(e.touches[1]);
    const newDist = pinchDist(e.touches[0], e.touches[1]);
    const newMid  = { x: (t0.x + t1.x) / 2, y: (t0.y + t1.y) / 2 };

    if (importState._pinchDist > 0) {
      // Zoom centrato sul midpoint
      zoomImportAt(newMid.x, newMid.y, newDist / importState._pinchDist);
    }

    // Pan simultaneo al pinch (segue lo spostamento del midpoint)
    if (importState._pinchMid) {
      importState.view.offX += newMid.x - importState._pinchMid.x;
      importState.view.offY += newMid.y - importState._pinchMid.y;
    }

    importState._pinchDist = newDist;
    importState._pinchMid  = newMid;
    _didPan = true;
  }
...
function onImportTouchEnd(e) {
  ...
  const wasTap  = !_didPan && elapsed < _TAP_MAX_MS && importState._lastTouches.length === 1;

  if (wasTap) {
    const pos = getTouchCanvasPos(importState._lastTouches[0]);
    registerCalibClick(pos.x, pos.y);
  }
  ...
  if (e.touches.length === 0) {
    // Tutte le dita sollevate: reset completo
    importState._pinchDist = 0;
    importState._pinchMid  = null;
    importState._panStart  = null;
    _mouseDownPos   = null;
    _didPan         = false;
    _touchStartTime = 0;
  } else if (e.touches.length === 1) {
    // Tornati a 1 dito dopo pinch: ricalcola panStart per evitare salto
    const pos = getTouchCanvasPos(e.touches[0]);
    _mouseDownPos = pos;
    _didPan = true; // non considerare come tap
    importState._panStart = {
      x: pos.x - importState.view.offX,
      y: pos.y - importState.view.offY,
    };
