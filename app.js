...
const MITER_TOL = 14;  // tolleranza aumentata per muri con assi diversi
...
/**
 * getMiterAt — trova il raccordo a mitra per un'estremità di un muro.
 * Versione robusta che gestisce:
 *   - Connessioni endpoint-to-endpoint (angolo L, angolo T, angolo +)
 *   - Connessioni endpoint-to-body (raccordo a T trasversale)
 *   - Snap a 90° forzato per muri quasi perpendicolari
 */
function getMiterAt(w, side, wallList) {
  ...
  // Direzione del muro verso l'esterno dal nodo
  ...
  for (const other of candidates) {
    ...
    let oDirX, oDirY;
    let isBodyConnection = false;

    if (dist(other.start, anchor) < MITER_TOL) {
      oDirX = oo.ux; oDirY = oo.uy;
    } else if (dist(other.end, anchor) < MITER_TOL) {
      oDirX = -oo.ux; oDirY = -oo.uy;
    } else {
      // Controlla se anchor cade sul corpo del muro (raccordo a T)
      const d = distPointToSegment(anchor, other.start, other.end);
      if (d.dist > MITER_TOL) continue;
      // Per raccordo a T: la direzione del muro traversale
      // viene determinata in base all'angolo tra i due muri
      isBodyConnection = true;
      // Scegli la direzione dell'altro muro che forma un angolo acuto con wDir
      const dot1 =  wDirX * oo.ux + wDirY * oo.uy;
      const dot2 = -wDirX * oo.ux - wDirY * oo.uy;
      if (Math.abs(dot1) > Math.abs(dot2)) { oDirX =  oo.ux; oDirY =  oo.uy; }
      else                                  { oDirX = -oo.ux; oDirY = -oo.uy; }
    }

    const dot = wDirX * oDirX + wDirY * oDirY;
    const clampedDot = Math.max(-1, Math.min(1, dot));
    let angleDiff = Math.abs(Math.acos(clampedDot));

    // Snap a 90°: se l'angolo è entro 5° da 90°, forza esattamente 90°
    const SNAP_90_RAD = 5 * Math.PI / 180;
    if (Math.abs(angleDiff - Math.PI / 2) < SNAP_90_RAD) {
      angleDiff = Math.PI / 2;
      // Forza la direzione perpendicolare esatta
      const perpX = -wDirY, perpY = wDirX;
      const dotPerp = oDirX * perpX + oDirY * perpY;
      if (dotPerp < 0) { oDirX = -perpX; oDirY = -perpY; }
      else             { oDirX =  perpX; oDirY =  perpY; }
    }

    // Escludi muri quasi paralleli o esattamente opposti (no raccordo sensato)
    if (angleDiff < 0.087 || angleDiff > Math.PI - 0.087) continue;

    // Per raccordo a T sul corpo, usiamo la normale del muro traversale
    let topP, botP, topD, botD;
    if (isBodyConnection) {
      // Ancora = punto di proiezione sul corpo del muro attraversato
      topP = { x: anchor.x + oo.nx, y: anchor.y + oo.ny };
      botP = { x: anchor.x - oo.nx, y: anchor.y - oo.ny };
      topD = { x: wDirX, y: wDirY };
      botD = { x: wDirX, y: wDirY };
    } else {
      topP = { x: anchor.x + oo.nx, y: anchor.y + oo.ny };
      botP = { x: anchor.x - oo.nx, y: anchor.y - oo.ny };
    ...
    const topIntersect = lineIntersect(
      { x: anchor.x + o.nx, y: anchor.y + o.ny }, { x: wDirX, y: wDirY },
      topP, topD
    );
    const botIntersect = lineIntersect(
      { x: anchor.x - o.nx, y: anchor.y - o.ny }, { x: wDirX, y: wDirY },
      botP, botD
    );

    if (topIntersect && botIntersect) {
      const maxDist = (o.thick + oo.thick) * 6;
      if (dist(topIntersect, anchor) < maxDist && dist(botIntersect, anchor) < maxDist) {
        if (angleDiff < bestAngleDiff) {
          bestAngleDiff = angleDiff;
          best = { top: topIntersect, bot: botIntersect, other, oo };
        }
      }
    }
  }
...
function buildNodeMap(walls) {
  // Griglia a 10px per unificare nodi geometricamente vicini
  const GRID = 10;
  ...
  // Aggiungi connessioni T: se l'endpoint di un muro cade sul corpo di un altro
  for (const w of walls) {
    for (const side of ['start', 'end']) {
      const anchor = w[side];
      for (const other of walls) {
        if (other.id === w.id) continue;
        // Salta se l'endpoint di 'other' è già vicino ad anchor (già rilevato sopra)
        if (dist(other.start, anchor) < GRID * 2 || dist(other.end, anchor) < GRID * 2) continue;
        const d = distPointToSegment(anchor, other.start, other.end);
        if (d.dist < MITER_TOL) {
          // Punto di intersezione sul corpo del muro
          ...
          if (entries && !entries.find(e => e.wall.id === other.id)) {
            // Aggiungi l'altro muro nel nodo con side sintetico
...
async function init() {
  ...
  // Import da PDF esistente
  document.getElementById('btn-import-pdf-home').addEventListener('click', () => {
    // Crea un nuovo progetto vuoto poi apri la schermata import
    const name  = 'Importazione PDF';
    const scale = 100;
    ...
    // Apri schermata import
    showScreen('import-screen');
    // Trigger file picker (con delay per transizione schermata)
    setTimeout(() => {
