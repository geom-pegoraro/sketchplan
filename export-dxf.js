...
function buildWallEntities(pxToMm) {
  ...
  for (const w of state.walls) {
    const o = getWallOutline(w); if (!o) continue;
    const ms = getMiterAt(w,'start',state.walls), me = getMiterAt(w,'end',state.walls);
    const tl=ms?ms.top:o.tl, bl=ms?ms.bot:o.bl, tr=me?me.top:o.tr, br=me?me.bot:o.br;
    const layer=dxfLayer('MURI',w.state||'existing'), color=dxfColor(w.state||'existing');
...
function buildWallWithOpenings(w,o,tl,tr,br,bl,openings,pxToMm,layer,color) {
  const out=[], dx=w.end.x-w.start.x, dy=w.end.y-w.start.y;
  const len=Math.sqrt(dx*dx+dy*dy); if(len<1) return out;
  ...
  for(const side of['top','bot']){
    ...
    for(const op of sorted){
      const oW=(op.width/100)*state.scale, half=oW/2/len;
      ...
      if(dist2(px,py,gsx,gsy)>0.5) out.push(...dxfLine(layer,color,px,py,gsx,gsy,pxToMm));
      px=gex; py=gey;
    }
    if(dist2(px,py,endPt.x,endPt.y)>0.5) out.push(...dxfLine(layer,color,px,py,endPt.x,endPt.y,pxToMm));
  }
...
function buildOpeningEntities(pxToMm) {
  ...
  for(const o of state.openings){
    const wall=state.walls.find(w=>w.id===o.wallId); if(!wall) continue;
    ...
    const oW=(o.width/100)*state.scale;
    const cpx=wall.start.x+o.t*dx, cpy=wall.start.y+o.t*dy;
    const hW=oW/2;
    const oSt=o.state||'existing', col=dxfColor(oSt);
    if(o.type==='door'){
      out.push(...buildDoorDXF(cpx,cpy,ux,uy,nx,ny,hW,thick,oW,o.flip||false,o.rotate||0,pxToMm,dxfLayer('PORTE',oSt),col));
    } else {
      out.push(...buildWindowDXF(cpx,cpy,ux,uy,nx,ny,hW,thick,pxToMm,dxfLayer('FINESTRE',oSt),col));
    }
  }
  return out;
}

function buildDoorDXF(cx,cy,ux,uy,nx,ny,hW,thick,doorW,flip,rotate,pxToMm,layer,color){
  layer=layer||'PORTE-ESISTENTE'; color=color!==undefined?color:7;
  const out=[], stipW=Math.min(thick*0.3,doorW*0.08);
  ...
  const sn=Math.sqrt(nx*nx+ny*ny)||1, bndx=nx/sn, bndy=ny/sn;
  const sndx=rotate?-bndx:bndx, sndy=rotate?-bndy:bndy;
  out.push(...dxfLwPolyline(layer,color,[{x:phx+nx,y:phy+ny},{x:phx-nx,y:phy-ny},{x:phx-nx+wdx*stipW,y:phy-ny+wdy*stipW},{x:phx+nx+wdx*stipW,y:phy+ny+wdy*stipW}],true,pxToMm));
  out.push(...dxfLwPolyline(layer,color,[{x:plx+nx,y:ply+ny},{x:plx-nx,y:ply-ny},{x:plx-nx-wdx*stipW,y:ply-ny-wdy*stipW},{x:plx+nx-wdx*stipW,y:ply+ny-wdy*stipW}],true,pxToMm));
  const hgx=phx+sndx*thick, hgy=phy+sndy*thick;
  out.push(...dxfLine(layer,color,hgx,hgy,hgx+sndx*doorW,hgy+sndy*doorW,pxToMm));
  ...
  out.push(...dxfArc(layer,color,hgx,hgy,doorW,a1,a2,pxToMm));
...
function buildWindowDXF(cx,cy,ux,uy,nx,ny,hW,thick,pxToMm,layer,color){
  ...
  const out=[], p0x=cx-ux*hW, p0y=cy-uy*hW, p1x=cx+ux*hW, p1y=cy+uy*hW;
  ...
  for(const f of[-0.6,0,0.6]) out.push(...dxfLine(layer,color,p0x+nx*f,p0y+ny*f,p1x+nx*f,p1y+ny*f,pxToMm));
  return out;
}
...
function buildColumnEntities(pxToMm){
  ...
  for(const c of(state.columns||[])){
    const layer=dxfLayer('PILASTRI',c.state||'existing'), color=dxfColor(c.state||'existing');
